    ...
    <div class="space-y-4 p-4">
      ...
      <div class="flex flex-wrap items-center gap-3 text-sm text-slate-600">
        ...
        <span class="inline-flex items-center gap-1">
          ...
          <span class="font-medium text-slate-700">{{ Number(hostel.rating || 0).toFixed(1) }}</span>
          <span>({{ hostel.total_reviews || 0 }} reviews)</span>
        </span>
      </div>

      <p class="text-sm text-slate-700">
        Starting from
        <span class="font-bold text-blue-900">GHS {{ Number(startingPrice).toLocaleString() }}</span>
...
const startingPrice = computed(() => {
  if (!Array.isArray(props.hostel.room_types) || props.hostel.room_types.length === 0) {
    return 0;
  }

  return Math.min(...props.hostel.room_types.map((roomType) => Number(roomType.price_per_semester)));
});
...
const goToHostel = () => {
  const detailIdentifier = props.hostel.slug ?? props.hostel.id;
  router.visit(`/hostels/${detailIdentifier}`);
};
</script>
